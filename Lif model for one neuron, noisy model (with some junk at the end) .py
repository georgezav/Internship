...
def f_I_noise(Vth, Vrest, Vreset, I, tau, sigma, n ):
    ...
    yt = (Vth - Vrest - I)/sigma
    yr = (Vreset - Vrest -I)/sigma
    v = 0 
    f = 0 
   
    for i in range(-1,n):
        if i == 0 or i == (n-1):                  
            v +=  tau * math.sqrt(math.pi) * ((yt-yr)/3*n)* ((Vth-Vreset)/sigma) * (math.e**((n)**2) * (1 + math.erf(n)))
        elif (i % 2) == 0:
            v += 2* (tau * math.sqrt(math.pi) *((yt-yr)/3*n)* ((Vth-Vreset)/sigma) * (math.e**((n)**2) * (1 + math.erf(n))))
        elif (i % 2) == 1:
            v += 4*(tau * math.sqrt(math.pi) *((yt-yr)/3*n)* ((Vth-Vreset)/sigma) * (math.e**((n)**2) * (1 + math.erf(n))))
